Public Class DeleteListMovies

    Private Sub DeleteListMovies_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub DataGridView_Movies_List_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles DataGridView_Movies_List.CellContentClick
        ...
        If e.RowIndex >= 0 Then
            ...
            Dim result As DialogResult = MessageBox.Show(userDetails.ToString() & vbCrLf & "Are you sure you want to delete this movie?", "Movie Details", MessageBoxButtons.YesNo, MessageBoxIcon.Question)

            If result = DialogResult.Yes Then
                ' User chose to update the user          
                Globals.MovieId = selectedRow.Cells("video_id").Value.ToString()
                Globals.MovieTitle = selectedRow.Cells("title").Value.ToString()

                Dim dbManagerMovie As New DBManagerMovie
                dbManagerMovie.DeleteMovieWithId(Globals.MovieId)

                MsgBox($"{Globals.MovieTitle} has been deleted.", MsgBoxStyle.Information, "Movie deleted")
